# Anomalías térmicas de la SST en el mar interior de Chiloé — Enero–Junio 2026

**Producto:** OSTIA SST L4 (METOFFICE-GLO-SST-L4, 0.05°) · REP (1993-2025) + NRT (2026)
**Período analizado:** 01 de January de 2026 → 28 de June de 2026 (179 días)
**Referencia climatológica:** 1993–2020
**Área:** Mar interior de Chiloé
(lat -43.6° – -41.4° S · lon -74.3° – -72.4° W)

---

## Síntesis

La temperatura superficial del mar en el mar interior de Chiloé se mantuvo en promedio
**+0.09 °C por encima de la climatología de referencia**
(1993–2020) durante el período enero–junio de 2026.

- **60 %** de los días presentó anomalía media positiva (más cálido que el
  promedio histórico del mismo período).
- El momento **más cálido** del período fue el **03 de February de 2026**,
  con una anomalía media de **+0.74 °C** sobre la climatología.
- El momento **más frío** fue el **16 de May de 2026**,
  con una anomalía de **-0.47 °C**.

## Ranking histórico enero–junio (1993–2026)

En comparación con el mismo período enero–junio de todos los años disponibles
(1993–2025 en REP + 2026 con NRT), **2026 ocupa el puesto 9 de 34**
en la escala de anomalía cálida. El año con la mayor anomalía positiva del registro
es **1998** (**+0.66 °C**).

### Top 5 años más cálidos (ene–jun)

| Año | Anomalía (°C) |
|-----|---------------|
| 1998 | +0.663 |
| 2017 | +0.516 |
| 2020 | +0.366 |
| 2016 | +0.304 |
| 2004 | +0.173 |

## Olas de calor marinas (MHW)

Se detectaron **1 evento(s) de ola de calor marina (MHW)** en el centroide de referencia (Chiloé Chulín), con un total de **7 días** en condición MHW (SST > percentil 90 climático por ≥ 5 días consecutivos) y una intensidad máxima de **+1.13 °C** sobre la climatología.

_(Definición Hobday et al. 2016: SST > percentil 90 climático durante ≥ 5 días
consecutivos; huecos de ≤ 2 días se unen. Centroide Chiloé Chulín, lat -42.63°,
lon -73.05°.)_

## Tendencia de largo plazo

La SST media del mar interior de Chiloé muestra una tendencia de
**-0.067 °C/década** en el registro REP (1993–2025), consistente con un
enfriamiento sostenido del océano en la región.

---

## Metodología

| Parámetro | Valor |
|-----------|-------|
| Producto REP | `METOFFICE-GLO-SST-L4-REP-OBS-SST` |
| Producto NRT | `METOFFICE-GLO-SST-L4-NRT-OBS-SST-V2` |
| Resolución espacial | 0.05° (~5.5 km) |
| Cobertura REP | 1993-01-01 → 2025-12-18 |
| Cobertura NRT | 2025-12-01 → 2026-06-28 |
| Baseline climatológico | 1993–2020 |
| Suavizado climatología | Ventana circular de 11 días (±5 días) |
| Suavizado GIF | Media móvil trailing de 7 días por celda |
| Detección MHW | Hobday et al. 2016 (p90, ≥ 5 días, gaps ≤ 2 días) |
| Generado | 2026-06-29 |

---
_Generado automáticamente por `run_chiloe_gif.py`_
