"""
Script puntual (one-off) para analizar anomalías de SST en una coordenada
específica, fuera de la grilla por regiones del pipeline principal
(01_download.py / 02_process.py / 03_visualize.py).
...
Reutiliza constantes de config.py (DATASET_ID, VARIABLE, FECHA_INICIO,
FECHA_FIN) — no las redefine.

Uso:
    python run_point.py --lat -42.6256807 --lon -73.0538369 --name chiloe_ancud
"""
import argparse
import os
import sys
...
# (ver CLAUDE.md), aunque FECHA_FIN de config.py sea posterior.
FECHA_FIN_DATASET = "2025-12-31"

POINTS_DATA_DIR = Path("data/points")
POINTS_FIGURES_DIR = Path("figures/points")
...
def parsear_argumentos():
    parser = argparse.ArgumentParser(
        description="Anomalías de SST en un punto específico (lat/lon)."
    )
    parser.add_argument("--lat", type=float, required=True, help="Latitud decimal (ej. -42.6256807)")
    parser.add_argument("--lon", type=float, required=True, help="Longitud decimal (ej. -73.0538369)")
    parser.add_argument("--name", type=str, default=None,
                        help="Nombre identificador del punto (default: derivado de lat/lon)")
...
def descargar_punto(nombre, lat, lon, fecha_inicio, fecha_fin, archivo_salida, data_dir):
    ...
    margen = 0.10
    print(f"→ Descargando punto {nombre} (lat={lat}, lon={lon}, "
    ...
    try:
        ...
        print(f"   ✗ Error al descargar punto {nombre}: {exc}")
        raise


def procesar_punto(archivo, lat, lon, baseline_start, baseline_end):
    ...
    if nan_frac > 0:
        print(f"   ⚠️  La celda tiene {nan_frac:.1%} de valores NaN (posible máscara de tierra/hielo).")
...
def main():
    ...
    data_dir = POINTS_DATA_DIR / nombre
    figuras_dir = POINTS_FIGURES_DIR / nombre
    data_dir.mkdir(parents=True, exist_ok=True)
    figuras_dir.mkdir(parents=True, exist_ok=True)

    archivo_nc = data_dir / f"point_{nombre}_sst.nc"

    print(f"📍 Punto: {nombre} (lat={args.lat}, lon={args.lon})")
    verificar_credenciales()
    descargar_punto(nombre, args.lat, args.lon, args.start, fecha_fin, archivo_nc, data_dir)

    print(f"→ Procesando {nombre}...")
    df, celda = procesar_punto(archivo_nc, args.lat, args.lon,
                               args.baseline_start, args.baseline_end)
    salida_csv = data_dir / f"point_monthly_{nombre}.csv"
    df.to_csv(salida_csv, index=False)
    print(f"   ✓ {salida_csv.name} ({len(df)} filas)")

    print("🎨 Generando figuras...")
    figura_serie_anomalia(df, nombre, args.baseline_start, args.baseline_end, figuras_dir)
    figura_heatmap_anomalia(df, nombre, figuras_dir)
    figura_anomalia_anual(df, nombre, figuras_dir)
